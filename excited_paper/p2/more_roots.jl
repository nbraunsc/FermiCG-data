...
#ctx = fcidump.read("/home/nbraunsc/FermiCG-data/excited_paper/p2/biphenyl-2mer-1d-fcidump");
#h = ctx["H1"];
#g = ctx["H2"];
#ecore = ctx["ECORE"];
...
#@save  "/home/nbraunsc/FermiCG-data/excited_paper/p2/cmf_diis.jld2" ints d1 clusters init_fspace
@load  "/home/nbraunsc/FermiCG-data/excited_paper/p2/cmf_diis.jld2"
ecore = ints.h0
...
#ci_vector[FermiCG.FockConfig(init_fspace)][FermiCG.ClusterConfig([6,1,1,1])] = zeros(Float64,nroots)
#ci_vector[FermiCG.FockConfig(init_fspace)][FermiCG.ClusterConfig([1,6,1,1])] = zeros(Float64,nroots)
#ci_vector[FermiCG.FockConfig(init_fspace)][FermiCG.ClusterConfig([1,1,6,1])] = zeros(Float64,nroots)
#ci_vector[FermiCG.FockConfig(init_fspace)][FermiCG.ClusterConfig([1,1,1,6])] = zeros(Float64,nroots)

#ci_vector[FermiCG.FockConfig(init_fspace)][FermiCG.ClusterConfig([7,1,1,1])] = zeros(Float64,nroots)
#ci_vector[FermiCG.FockConfig(init_fspace)][FermiCG.ClusterConfig([1,7,1,1])] = zeros(Float64,nroots)
#ci_vector[FermiCG.FockConfig(init_fspace)][FermiCG.ClusterConfig([1,1,7,1])] = zeros(Float64,nroots)
#ci_vector[FermiCG.FockConfig(init_fspace)][FermiCG.ClusterConfig([1,1,1,7])] = zeros(Float64,nroots)
...
for i in thresh
    ...
    @save "/home/nbraunsc/FermiCG-data/excited_paper/p2/"*string(name) clusters d1 ints cluster_bases ci_vector e0 v0 e2 ecore s2
end
